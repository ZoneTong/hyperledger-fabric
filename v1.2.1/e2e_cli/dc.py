...
for index in range(len(services)):
    if services[index].startswith('peer'):
        services[index] = services[index]+'.health.bgiblockchain.com'
    elif services[index].startswith('orderer') or services[index].startswith('kafka') or services[index].startswith('zookeeper'):
        services[index] = services[index]+'.bgiblockchain.com'
s_services = ''
for ser in services:
    s_services += ' ' + ser
...
if node == 'loop':
    ...
    cmd = 'IMAGE_TAG=amd64-1.2.1 docker-compose --project-name=e2ecli -f docker-compose-'+node+'.yaml '+subcmd + s_services
    if node == 'orderer':
    	print os.popen(cmd).readlines()
