...
export PATH=${PWD}/../../release/${OS_ARCH}/bin:${PWD}/../bin:${PWD}:$PATH
...
function printHelp () {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"bgihealth\")"
  ...
  echo
  echo "	eyfn.sh generate -c bgihealth"
  echo "	eyfn.sh up -c bgihealth -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c bgihealth"
  echo
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images|awk '($1 ~ /dev-peer.*.token_.*/) {print $3}')
...
function networkUp () {
  ...
  echo "###############################################################"
  docker exec cli2 ./scripts/step3org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function createConfigTx () {
  ...
  echo "###############################################################"
  docker exec cli2 scripts/step1org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  cp -r crypto-config/peerOrganizations     org3-artifacts/crypto-config/
  echo
}
...
#default for delay
CLI_DELAY=3
# channel name defaults to "bgihealth"
