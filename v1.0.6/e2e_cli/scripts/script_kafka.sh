...
createChannel() {
	setGlobals 0
		...
		peer channel create -o orderer0.businessblockchain.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -t 30 >&log.txt
	fi
	res=$?
	cat log.txt
