...
and two Peer Orgs (``Health`` & ``Org2``) each managing and maintaining two peer nodes.
...
.. code:: bash
  ...
  #################################################################
  #######    Generating anchor peer update for HealthMSP   ##########
  #################################################################
...
Define the anchor peer for Health on the channel:

.. code:: bash

    # make sure to set the <channel-ID> parm
    ./../../release/$os_arch/bin/configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/HealthMSPanchors.tx -channelID <channel-ID> -asOrg HealthMSP
...
.. code:: bash
  ...
  ===================== Query on PEER3 on channel 'bgilifechain' is successful =====================
...
script will use a default name of ``bgilifechain``.  The cli timeout parameter
...
endorsement from one peer node that is a part of either ``Health`` or ``Org2``.
The command is:
...
You should see a database named ``bgilifechain`` (or your unique channel name) and
